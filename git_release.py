...
Usage:
    python git_release.py --version 0.2.4 \
        --message "Enhanced pagination and currency persistence"
...
class GitReleaseManager:
    ...
    def update_version_file(self, new_version: str):
        ...
        if self.dry_run:
            msg = f"Would update pyproject.toml version to {new_version}"
        ...
        msg = f"Updated pyproject.toml version to {new_version}"
    ...
    def create_commit(self, message: str, files: Optional[List[str]] = None):
        ...
        try:
            result = self.run_command(["git", "diff", "--cached", "--quiet"],
                                      check=False)
    ...
    def push_changes(self, tag: Optional[str] = None):
    ...
    def create_github_release(self, tag: str, title: str, message: str):
        ...
        try:
            ...
            msg = ("GitHub CLI (gh) not available. "
...
def main():
    ...
    version_group.add_argument(
        "--version", help="Specific version to tag (e.g., 0.2.4)")
    version_group.add_argument(
    ...
    try:
        ...
        msg = f"Current branch: {current_branch}"
        release_manager.print_status(msg, "INFO")

        if has_changes:
            msg = f"Found {len(changed_files)} changed files:"
            release_manager.print_status(msg, "WARNING")
            for file in changed_files[:10]:  # Show first 10 files
                release_manager.print_status(f"  {file}", "INFO")
            if len(changed_files) > 10:
                msg = f"  ... and {len(changed_files) - 10} more"
                release_manager.print_status(msg, "INFO")
        ...
        if args.version:
            ...
            new_version = release_manager.bump_version(
        ...
        if has_changes and (args.commit_all or new_version != current_version):
            ...
            files_to_commit = (
                ["pyproject.toml"] if not args.commit_all else None)
            release_manager.create_commit(commit_message, files_to_commit)
        elif has_changes and not args.commit_all:
            msg = ("Uncommitted changes found. "
                   "Use --commit-all to include them.")
            release_manager.print_status(msg, "WARNING")
        ...
        if args.create_release:
            ...
            release_notes = release_manager.generate_release_notes(
                new_version, args.message)
            release_manager.create_github_release(
                tag, release_title, release_notes)

        msg = "Release process completed successfully!"
        release_manager.print_status(msg, "SUCCESS")
        release_manager.print_status(f"Created release: {tag}", "SUCCESS")

        if not args.push:
            push_msg = (f"Remember to push your changes: "
                        f"git push && git push origin {tag}")
            release_manager.print_status(push_msg, "INFO")

        if not args.create_release:
            gh_msg = ("To create GitHub release: "
                      "python git_release.py --create-release")
            release_manager.print_status(gh_msg, "INFO")
