...
class StockBrowserWindow:
    ...
    def __init__(self, parent: tk.Tk, api_client: LCSCClient, config: Config):
        ...
        title = f"Stock Browser (Debug) - {config.app_name} v{config.version}"
        self.window.title(title)
        # More reasonable default size for split screen
        ...
        # Bind keyboard shortcuts for fullscreen
        # (minimize uses standard Windows shortcut)
        ...
        status_text = (
            "Ready. Enter search term or click 'List All' " "to load products."
        )
        ...
        # Initialize currency column headers with saved currency
        self._update_price_column_headers(self.current_currency.get())
    ...
    def _create_widgets(self):
        ...
        # Info label
        info_text = (
            "Browse components available in the mock server. "
            "Search in code, model, name, and description."
        )
        ...
        ttk.Label(search_frame, text="Keyword:").grid(
        ...
        ttk.Label(search_frame, text="History:").grid(
        ...
        clear_history_button = ttk.Button(
            search_frame, text="Clear History", command=self._clear_history
        )
        ...
        result_label = ttk.Label(
            search_frame, textvariable=self.result_count_var
        )
        result_label.grid(row=1, column=0, columnspan=2, padx=5, sticky="w")

        # Currency selection
        ttk.Label(search_frame, text="Currency:").grid(
            row=1, column=2, padx=(10, 5)
        )
        ...
        rate_info_label = ttk.Label(
            search_frame, textvariable=self.rate_info_var, font=("Arial", 8)
        )
        ...
        tree_frame.grid(
            row=3, column=0, columnspan=3, sticky="nsew", pady=(0, 10)
        )
        ...
        # Define columns - ALL 10 possible price breaks
        # (no image column for speed)
        ...
        ttk.Label(left_frame, textvariable=self.page_info_var).pack(
            side=tk.LEFT, padx=5
        )

        ttk.Button(left_frame, text="⏮️ First", command=self._first_page).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(
            left_frame, text="◀ Previous", command=self._previous_page
        ...
        ttk.Button(left_frame, text="Next ▶", command=self._next_page).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(left_frame, text="Last ⏭️", command=self._last_page).pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Label(right_frame, text="Page size:").pack(
        ...
        button_frame.grid(
            row=6, column=0, columnspan=3, pady=(15, 10), sticky="ew"
        )
        ...
        ttk.Button(
            button_frame, text="🔄 Refresh", command=self._load_products
        ).pack(side=tk.LEFT, padx=5)
        ttk.Button(
            button_frame,
            text="💾 Export CSV",
    ...
    def _load_products(self, keyword: Optional[str] = None):
        ...
        try:
            ...
            self.status_var.set(
                f"Loaded {len(self.products)} products (Page {self.current_page}/{self.total_pages})"
            )
            ...
            logger.info(
                f"Loaded {len(self.products)} products from mock server"
            )

        except Exception as e:
            # Show error dialog (no modal grab needed)
            messagebox.showerror(
                "Error", f"Failed to load products:\n{str(e)}"
            )
    ...
    def _populate_tree(self):
        ...
        # Ensure price column headers show the current currency
        self._update_price_column_headers(self.current_currency.get())
        ...
        for product in self.products:
            ...
            price_dict = {
                tier.quantity: tier.unit_price for tier in product.price_tiers
            }
    ...
    def _clear_history(self):
        ...
        self._update_history_dropdown()
        messagebox.showinfo(
            "History Cleared", "Search history has been cleared."
        )
    ...
    def _list_all_stock(self):
        ...
        try:
            ...
            self._update_pagination_info()
            status_msg = (
                f"✅ Listed all stock: Showing {len(self.products)} products "
            ...
            logger.info(
                f"Listed all stock: {total} total products, showing page {self.current_page}/{self.total_pages}"
            )

        except Exception as e:
            # Show error dialog (no modal grab needed)
            messagebox.showerror(
                "Error", f"Failed to list all stock:\n{str(e)}"
            )
    ...
    def _change_page_size(self, event):
        ...
        logger.info(
            f"Changing page size from {self.page_size} to {new_page_size}"
        )
    ...
    def _create_page_buttons(self):
        ...
        # Debug logging
        debug_msg = (
            f"Creating page buttons: current={self.current_page}, "
            f"total={self.total_pages}, range=[{start_page}-{end_page}]"
        )
    ...
    def _sort_by_column(self, column: str):
        ...
        if column == "Stock":
            ...
            items_with_price.sort(
                key=lambda x: x[0], reverse=not self.sort_reverse
            )

            # Combine: items with price first, then items without price (always at the end)
            items = [
                (x[1], x[2]) for x in items_with_price
            ] + items_without_price
        else:
            # Sort as string
        ...
        self.status_var.set(
            f"Sorted by {column} ({'descending' if self.sort_reverse else 'ascending'})"
        )
    ...
    def _show_details(self, event):
        ...
        # Handle both proper Unicode and double-encoded characters
        description_text = (
            description_text.replace("Ω", "ohm")
            .replace("Î©", "ohm")
            .replace("±", "+/-")
            .replace("Â±", "+/-")
            .replace("µ", "u")
...
"""
        ...
        for tier in product.price_tiers:
            details_text += (
                f"{tier.quantity}+: ${tier.unit_price:.4f} {tier.currency}\n"
            )
        ...
        text_height = min(
            lines + 2, 30
        )  # Cap at 30 lines to prevent giant windows
        ...
        if product.datasheet_url:
            ...
            ttk.Label(
                datasheet_frame, text="Datasheet:", font=("Arial", 10, "bold")
            ).pack(side=tk.LEFT, padx=(0, 10))

            # Create PDF icon button
            try:
                pdf_icon = self._create_pdf_icon(24)
                # Keep reference to prevent garbage collection by storing
                # in local var (icon_ref kept alive by button widget)

                pdf_button = tk.Button(
                    datasheet_frame,
                    image=pdf_icon,
                    command=lambda: self._open_datasheet(
                ...
                url_label = ttk.Label(
                    datasheet_frame,
                    text="Click PDF icon to open datasheet",
                    foreground="#666",
                    font=("Arial", 8),
                )
                url_label.pack(side=tk.LEFT)

            except Exception:  # Fallback if PDF icon fails
                ...
                link_label.bind(
                    "<Button-1>",
                    lambda e: self._open_datasheet(product.datasheet_url),
                )

        # Show image if available
        if product.image_url:
            image_frame = ttk.LabelFrame(
                main_frame, text="Component Image", padding="10"
            )
    ...
    def _open_datasheet(self, url):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Failed to open datasheet:\n{str(e)}"
            )
    ...
    def _export_csv(self):
        ...
        default_filename = (
            f"mock_server_stock_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        )
        ...
        try:
            with open(filename, "w", newline="", encoding="utf-8") as csvfile:
                ...
                for product in self.products:
                    # Get prices from price tiers for all 10 possible
                    # quantities
                    price_breaks = {
                        1: "",
                        10: "",
                    ...
                    for tier in product.price_tiers:
                        if tier.quantity in price_breaks:
                            price_breaks[tier.quantity] = (
                                f"{tier.unit_price:.4f}"
                            )
            ...
            messagebox.showerror(
                "Export Failed", f"Failed to export CSV:\n{str(e)}"
            )
        ...
        try:
            if self.is_fullscreen:
                ...
                if self.normal_geometry:
                    ...
                    self.status_var.set(
                        f"✅ Restored geometry: {self.normal_geometry}"
                    )
                    # Schedule clearing the message after 3 seconds
                    self.window.after(
                        3000, lambda: self.status_var.set("Ready")
                    )
                ...
                stored_geom = self.normal_geometry
                status_msg = f"Entering fullscreen. Stored geometry: {stored_geom}"
                self.status_var.set(status_msg)
